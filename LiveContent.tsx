...
export const LiveContentProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const fetchData = async () => {
    ...
    // Si pas d'URL ou URL placeholder, on reste en mode local sans erreur
    ...
    // safeFetch permet d'éviter que tout le site plante si une seule table manque
    const safeFetch = async (query: any) => {
        try {
            const { data, error } = await query;
            if (error) {
                console.warn("Erreur chargement table:", error.message);
                return []; 
            }
            return data;
        } catch (e) {
    ...
    try {
      ...
      // On ne met une erreur visible que si vraiment aucun produit n'a chargé
      if (products.length === 0) setError("Connexion instable");
    } finally {
      setLoading(false);
    }
  };
